...
const ResortCard = ({ resort }: { resort: Resort }) => (
  <div className="group bg-card rounded-xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1">
    <div className="relative overflow-hidden aspect-[4/3]">
      <img src={resort.image_url || "/placeholder.svg"} alt={resort.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
      <div className="absolute top-3 right-3 bg-card/90 backdrop-blur-sm rounded-lg px-2.5 py-1 text-xs font-semibold text-foreground">
        ₹{Number(resort.price_per_night).toLocaleString("en-IN")}/night
      </div>
    </div>
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1 text-xs">
        ...
        <span className="text-muted-foreground">{resort.location || "Location TBD"}</span>
      </div>
      ...
      <div className="flex items-center justify-between pt-2">
        <div className="flex items-center gap-1">
          ...
          <span className="text-sm font-semibold text-foreground">{resort.rating ?? 0}</span>
          <span className="text-xs text-muted-foreground">({resort.reviews_count ?? 0})</span>
        </div>
