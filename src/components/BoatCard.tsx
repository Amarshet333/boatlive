...
const BoatCard = ({ boat }: { boat: Boat }) => (
  <div className="group bg-card rounded-xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1">
    <div className="relative overflow-hidden aspect-[4/3]">
      <img src={boat.image} alt={boat.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
      <div className="absolute top-3 right-3 bg-card/90 backdrop-blur-sm rounded-lg px-2.5 py-1 text-xs font-semibold text-foreground">
        ₹{boat.price.toLocaleString("en-IN")}
      </div>
    </div>
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1 text-xs">
        ...
        <span className="text-muted-foreground">{boat.location}</span>
      </div>
      <div className="flex items-center justify-between pt-2">
        <div className="flex items-center gap-1">
          <Star className="h-4 w-4 fill-primary text-primary" />
          <span className="text-sm font-semibold text-foreground">{boat.rating}</span>
          <span className="text-xs text-muted-foreground">({boat.reviews})</span>
        </div>
