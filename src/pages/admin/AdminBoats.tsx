...
const emptyBoat = {
  ...
  location: "", features: [] as string[], is_active: true, approved: true,
};
...
const AdminBoats = () => {
  ...
  const openEdit = (b: Boat) => {
    ...
    setForm({
      ...
      is_active: b.is_active, approved: b.approved,
    });
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      ...
      if (error) toast.error(error.message); else toast.success("Boat added");
    }
    setSaving(false);
    setOpen(false);
    fetchBoats();
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error) toast.error(error.message); else { toast.success("Deleted"); fetchBoats(); }
  };

  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-2xl font-bold text-foreground">Manage Boats</h1>
            ...
            <tbody>
              {boats.map((b) => (
                <tr key={b.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                  ...
                  <td className="p-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${b.approved && b.is_active ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                      {b.approved && b.is_active ? "Live" : "Draft"}
                    </span>
                  </td>
