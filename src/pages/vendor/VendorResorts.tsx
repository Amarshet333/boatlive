...
const emptyResort = {
  ...
  room_types: [] as string[], facilities: [] as string[], is_active: true,
};
...
const VendorResorts = () => {
  ...
  const fetchResorts = async () => {
    if (!vendorId) return;
    const { data } = await supabase.from("resorts").select("*").eq("vendor_id", vendorId).order("created_at", { ascending: false });
    setResorts(data ?? []);
    setLoading(false);
  };

  useEffect(() => { fetchResorts(); }, [vendorId]);
  ...
  const openEdit = (r: Resort) => {
    ...
    setForm({
      ...
      location: r.location ?? "", room_types: r.room_types ?? [], facilities: r.facilities ?? [],
  ...
  const handleSave = async () => {
    if (!form.name || !form.price_per_night || !vendorId) { toast.error("Name and price are required"); return; }
    setSaving(true);
    ...
    if (editing) {
      ...
      if (error) toast.error(error.message); else toast.success("Resort submitted for approval");
    }
    setSaving(false);
    setOpen(false);
    fetchResorts();
  ...
  return (
    <VendorLayout>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-2xl font-bold text-foreground">My Resorts</h1>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div><label className="text-sm font-medium">Room Types (comma separated)</label><Input value={roomTypesStr} onChange={(e) => setRoomTypesStr(e.target.value)} placeholder="Standard, Deluxe, Suite" /></div>
              <div><label className="text-sm font-medium">Facilities (comma separated)</label><Input value={facilitiesStr} onChange={(e) => setFacilitiesStr(e.target.value)} placeholder="Pool, WiFi, Parking" /></div>
              <div>
